...
const UploadedFileItem = React.memo(({ file }: UploadedFileProps) => {
  ...
  console.log('UploadedFileItem组件渲染了');
  const handlePreview = () => {
    console.log(url);
    window.open(url);
  };
  return (
    <div className="relative w-full max-w-2xl mx-auto my-1 px-1 border border-gray-200 rounded-lg bg-white">
      <div className="flex my-1">
        ...
        <div
          className="flex items-center justify-between p-2 border-b border-gray-100 last:border-b-0 cursor-pointer"
          onClick={handlePreview}
        >
          <span className={`text-sm font-medium ${nameColor}`}>{name}</span>
        </div>
        <button className="relative ml-auto px-1 hover:bg-gray-100 rounded flex items-center">
